    ...
    if request.method == 'POST':
        try:
            ...
            with sqlite3.connect('database.db') as con:
                cur = con.cursor()
                cur.execute("INSERT INTO applicant (name, addr, city, mobile, email) VALUES (?,?,?,?)",(nm, addr, city, zip))
...
def editrec():
    ...
    if request.method == 'POST':
        try:
            ...
            con.rollback()
            msg = "Error in the Edit: UPDATE applicants SET name="+nm+", addr="+addr+", city="+city+", mobile="+mobilenumber+" , email="+email+",WHERE rowid="+rowid
